...
const Hhome = () => {
  ...
  useEffect(()=>{
    ...
    const sgeo=new THREE.PlaneGeometry(10,10,100,100);
    ...
    function disturb(){
      const _position=sgeo.attributes.position;
            for(let i=0;i<_position.array.length;i++){
              _position.array[i*3]+=Math.random()*2-1;
                _position.array[i*3+1]+=Math.random();
                _position.array[i*3+2]+=Math.random()*2;
            }
            for(let i=0;i<_position.array.length;i++){
                _position.array[i*3]+=(_oposition[i*3]-_position.array[i*3])*0.001;
                _position.array[i*3+1]+=(_oposition[i*3+1]-_position.array[i*3+1])*0.002;
                _position.array[i*3+2]+=(_oposition[i*3+2]-_position.array[i*3+2])*0.001;
            }
